...
struct aws_credentials *aws_crt_credentials_new(
  ...
  return aws_credentials_new(
      allocator, aws_byte_cursor_from_c_str(access_key_id),
      aws_byte_cursor_from_c_str(secret_access_key),
      aws_byte_cursor_from_c_str(session_token), expiration_timepoint_seconds);
}

struct aws_byte_cursor aws_crt_credentials_get_access_key_id(
    const struct aws_credentials *credentials) {
  return aws_credentials_get_access_key_id(credentials);
}

struct aws_byte_cursor aws_crt_credentials_get_secret_access_key(
    const struct aws_credentials *credentials) {
  return aws_credentials_get_secret_access_key(credentials);
}

struct aws_byte_cursor aws_crt_credentials_get_session_token(
    const struct aws_credentials *credentials) {
  return aws_credentials_get_session_token(credentials);
}

uint64_t aws_crt_credentials_get_expiration_timepoint_seconds(
    const struct aws_credentials *credentials) {
